...
#define QUIT_LULL_THRESHOLD 3.0f
...
int comparePoints(const void *arg1, const void * arg2) {
    ...
    if (player_points[a] == player_points[b]) return 0;
    if (player_points[a] < player_points[b]) return -1;
...
void minigame_init()
{
    ...
    xm64player_open(&bg_music, "rom:/examplegame/super_chicken.xm64");

    for (int i = 0; i < 4; i++) {
        wav64_open(&sfx_mash[i], "rom:/examplegame/mash.wav64");
    }
...
void minigame_fixedloop(float deltatime)
{
    ...
    if (!isPaused) {
        ...
        if (can_control() && quit_lull_timer < QUIT_LULL_THRESHOLD) {
            quit_lull_timer += deltatime;
        }
...
void minigame_loop(float deltatime)
{
    if (can_control()) {
        ...
        for (size_t i = 0; i < core_get_playercount(); i++)
        {
            ...
            else if (btn.z && isPaused && i == pauser) {
                if (quit_lull_timer >= QUIT_LULL_THRESHOLD) {
                    int ranking[4] = {0, 1, 2, 3};

                    int curr_pauser = 3;
                    int maxScore = -1;
    
                    qsort(ranking, 4, sizeof(int), comparePoints);
                    
                    // no cheating quitter. quitters always lose
                    if (ranking[curr_pauser] == pauser) {
                        curr_pauser--;
                    }
    
                    maxScore = player_points[ranking[curr_pauser]];
                    core_set_winner(ranking[curr_pauser]);
                    curr_pauser--;
    
                    // workaround for unused value warning
                    for (curr_pauser = curr_pauser; curr_pauser >= 0; curr_pauser--) {
                        if (ranking[curr_pauser] == pauser) {
                            continue;
                        }
                        if (player_points[ranking[curr_pauser]] == maxScore) {
                            core_set_winner(ranking[curr_pauser]);
                        } else break;
                    }
    ...
    if (is_countdown()) {
        ...
        }, FONT_BUILTIN_DEBUG_MONO, 0, 100, "Paused\nPress Z to quit");
    }
...
{
    xm64player_close(&bg_music);
    for (int i = 0; i < 4; i++) {
        wav64_close(&sfx_mash[i]);
    }
