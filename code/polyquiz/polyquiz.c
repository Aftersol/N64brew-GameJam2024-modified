...
int expansionPackFactor;

rspq_block_t *poly = NULL;
sprite_t *bkg[NUM_BKGS];
rdpq_font_t *font = NULL;

xm64player_t background_music;

wav64_t sfx_start;
wav64_t sfx_confirm[4];

wav64_t sfx_drumroll;
...
void minigame_init()
{
    expansionPackFactor = (is_memory_expanded()) ? 1 : 2;
    ...
    xm64player_open(&background_music, "rom:/polyquiz/drozerix_sexxy_bit.xm64");
    xm64player_play(&background_music, 0);
    ...
    for (int i = 0; i < 4; i++) {
        wav64_open(&sfx_confirm[i], "rom:/core/menu_confirm.wav64");
    }

    wav64_open(&sfx_winner, "rom:/core/Winner.wav64");
    wav64_open(&sfx_draw, "rom:/core/Crash.wav64");
...
{
    for (int i = 0; i < 4; i++) {
        wav64_close(&sfx_confirm[4]);
    }
    
    wav64_close(&sfx_winner);
    wav64_close(&sfx_draw);

    wav64_close(&sfx_start);
    wav64_close(&sfx_drumroll);

    xm64player_close(&background_music);
...
void minigame_fixedloop(float dt)
{
    ...
    case GS_FADEOUT:
        ...
        if (state_time <= 0) {
            if (winner >= 0 && winner < 4) {
                wav64_play(&sfx_winner, 30);
            } else {
                wav64_play(&sfx_draw, 30);
            }
    ...
    if (all_confirmed || state_time <= 0) {
        if (state == GS_PLAY) {
            xm64player_stop(&background_music);
            wav64_play(&sfx_drumroll, 31);
        }
...
void minigame_loop(float dt)
{
    ...
    if (state == GS_PLAY) {
        ...
        for (int i=0; i<num_humans; i++) {
            ...
            if (btn.d_up || joypad_get_axis_held(JOYPAD_PORT_1+i, JOYPAD_AXIS_STICK_Y) == 1) player[i].guess++;
            if (btn.d_down || joypad_get_axis_held(JOYPAD_PORT_1+i, JOYPAD_AXIS_STICK_Y) == -1) player[i].guess--;
            if (btn.d_left || joypad_get_axis_pressed(JOYPAD_PORT_1+i, JOYPAD_AXIS_STICK_X) == -1) player[i].guess -= 10;
            if (btn.d_right || joypad_get_axis_pressed(JOYPAD_PORT_1+i, JOYPAD_AXIS_STICK_X) == 1) player[i].guess += 10;
    ...
    switch (state) {
    case GS_FADEIN:
        if (expansionPackFactor == 2) {
            rdpq_text_printf(&(rdpq_textparms_t){
                .width = display_get_width(), .align = ALIGN_CENTER,
            }, FONT_TEXT, 0, (50 / expansionPackFactor) + 10, "Guess the number\nof faces!");
            break;
        }
        rdpq_text_printf(&(rdpq_textparms_t){
            .width = display_get_width(), .align = ALIGN_CENTER,
    ...
    case GS_PLAY: {
        ...
        if (expansionPackFactor == 2) {
            rdpq_text_printf(NULL, FONT_TEXT, 220 / expansionPackFactor, (50 / expansionPackFactor) + 8, "%c", buf[0]);
            rdpq_text_printf(NULL, FONT_TEXT, 260 / expansionPackFactor, (50 / expansionPackFactor) + 8, "%c", buf[1]);
            rdpq_text_printf(NULL, FONT_TEXT, 310 / expansionPackFactor, (50 / expansionPackFactor) + 8, "%c", ':');
            rdpq_text_printf(NULL, FONT_TEXT, 340 / expansionPackFactor, (50 / expansionPackFactor) + 8, "%c", buf[3]);
            rdpq_text_printf(NULL, FONT_TEXT, 380 / expansionPackFactor, (50 / expansionPackFactor) + 8, "%c", buf[4]);
            break;
        }
    ...
    case GS_RESULT:
        rdpq_text_printf(&(rdpq_textparms_t){
            .width = display_get_width(), .align = ALIGN_CENTER,
        }, FONT_TEXT, 0, (50 / expansionPackFactor) + 5, "Faces: %d", num_faces);

        if (winner >= 0 && winner < 4) {
            rdpq_text_printf(&(rdpq_textparms_t){
                .width = display_get_width(), .align = ALIGN_CENTER,
                .style_id = 1,
            }, FONT_TEXT, 0, 240 / expansionPackFactor, "Player %d wins!", winner+1);
        } else {
            rdpq_text_printf(&(rdpq_textparms_t){
                .width = display_get_width(), .align = ALIGN_CENTER,
                .style_id = 1,
            }, FONT_TEXT, 0, 240 / expansionPackFactor, "Nobody wins");
        }
        break;
    default:
        break;
    ...
    if (state != GS_FADEIN) {
        for (int i=0; i<4; i++) {
            ...
            if (expansionPackFactor == 2) {
                rdpq_text_printf(&parms, FONT_TEXT, (100+i*140) / expansionPackFactor, (460 / expansionPackFactor) - 5, "%d", player[i].guess);
            } else {
